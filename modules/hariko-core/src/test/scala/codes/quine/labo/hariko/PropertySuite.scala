...
object PropertySuite extends SimpleTestSuite {
  ...
  test("Property.check") {
    val result1 = Property.checkWith(Param(0))((x: Boolean) => x || !x)
    assertEquals(result1, Property.Pass(0, 100))
    assertEquals(result1.toString, "pass (seed: 0x0, test: 100)")

    val result2 = Property.checkWith(Param(0))((x: Boolean) => x && !x)
    ...
    val result4 = Property.checkWith(Param(0, timeout = Duration("100ms"))) { (_: Unit) =>
      Thread.sleep(100)
      true
    }
    ...
    val result5 = Property.checkWith(Param(0, minScale = 100, timeout = Duration("150ms"))) { (_: Int) =>
      Thread.sleep(100)
      false
    }
    ...
    val result6 = Property.checkWith(Param(0)) { (_: Boolean) => sys.error("test") }
    assertEquals(
      result6.toString,
    ...
    val result7 = Property.checkWith(Param(0, minScale = 100, timeout = Duration("150ms"))) { (_: Int) =>
      Thread.sleep(100)
      sys.error("test")
    }
  ...
  test("Property.shrinkCounterExample") {
    ...
    // In this exceptional case, it returs a result immediately as discarded.
    assertEquals(
