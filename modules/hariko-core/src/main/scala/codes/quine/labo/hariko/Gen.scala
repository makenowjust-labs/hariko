...
trait Gen[T] {
  ...
  /** Selects the first valid generated value from this generator.
    *
    * It tries to generate values at most `param.maxDiscarded` times. If there is no valid value in these values, it
    * returns `None` instead.
    *
    * NOTE: This result tree is collapsed, so it makes a shirinking poor when this method is used in generator
    * implementation.
    */
  ...
  /** Like `gen.filter(p)`, but it is combined with `map`.
    *
    * Filtering is handled via `Option` instead of `Boolean` such that output type `U` can be different than input type
    * `T`.
    */
...
object Gen {
  ...
  /** Builds a two generators product with the mapping.
    ...
    * }}}
    *
    * NOTE: this and `tuple2` are associative. In short, it holds `Gen.map2(Gen.tuple2(gen1, gen2), gen3) { case ((x,
    * y), z) => (x, y, z) } == Gen.map2(gen1, Gen.tuple2(gen2, gen3)) { case (x, (y, z)) => (x, y, z) }`.
    *
    * @group combinator
    */
  ...
  /** Builds a new generator which choices a value according to distribution.
    *
    * For example, `Gen.frequency(1 -> genA, 2 -> genB)` choices `genA` and `genB` values according to `1:2` ratio.
    *
    * Example:
    *
    * {{{
  ...
  /** A map generator.
    *
    * Keys are generated by `keyGen`, and values are generated by `valueGen`. A generated map's size is in `sizeRange`.
    *
    * Example:
    *
    * {{{
  ...
  /** An option generator.
    *
    * It generates `None` and `Some(x)` according to `1:9` ratio. `gen` is generator for `Some(x)` values.
    *
    * Example:
    *
    * {{{
  ...
  /** An either generator.
    *
    * It generates `Left(x)` and `Right(x)` according to `1:1` ratio. `leftGen` is generator for `Left(x)` values, and
    * `rightGen` is generator for `Right(x)` values.
    *
    * Example:
    *
    * {{{
